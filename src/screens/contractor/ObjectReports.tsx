...
interface ObjectReportsProps {
  objectName: string;
  reports: ObjectReport[];
  onBack: () => void;
}

export function ContractorObjectReports({ objectName, reports, onBack }: ObjectReportsProps) {
  return (
    <div className="space-y-4">
      ...
      <Card className="glass-panel border-white/25 bg-gradient-to-br from-white/14 via-white/10 to-white/5 text-white shadow-[0_24px_70px_rgba(6,17,44,0.52)] backdrop-blur-[30px]">
        <CardHeader className="pb-2 sm:pb-3">
          <CardTitle className="text-[18px] font-semibold tracking-wide text-white sm:text-[20px]">Список отчётов</CardTitle>
        </CardHeader>
